			    ...
			    struct arm_smccc_res *res)
{
	...
	while(atomic_read(&STR) != 0)
	{
		...
		{
			pr_err("%s:%d %s(%u)get STR lock timeout happened\n",
				__func__,__LINE__,
...
int optee_config_ramlog(optee_invoke_fn *invoke_fn, struct tee_device *teedev)
{
	...
	pr_info("[OPTEE][RAMLOG] VA %pK, PA %pK, size 0x%x byte\n",
...
{
	pr_info("%s(%d): Notify TEE CPU to Back for STR CPU %s\n",
                                    ...
                                    size_t count, loff_t *ppos)
{
	...
	pr_err("\033[0;32;31m [RAMLOG] %s addr = 0x%x , length = %d\033[m\n",
