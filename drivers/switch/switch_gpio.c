...
static int gpio_switch_probe(struct platform_device *pdev)
{
	...
	if (dn)
	{
		if ((0 != of_property_read_string(dn, "switch-name", &dn_switch_name)) || (0 != of_property_read_u32(dn, "switch-gpio", &dn_switch_gpio)))
		{
	...
	printk("[switch-gpio] name: %s, gpio: %d\n", dn_switch_name, dn_switch_gpio);
