...
static ssize_t uart_switch_sysfs_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	uart_misc_debug("store uart_switch_number 0x%02x \n", uart_switch_number);
...
#define SW_SZ 4
	...
	uart_misc_debug("show uart_switch_number %X \n", uart_switch_number);
...
static ssize_t uart_tx_enable_sysfs_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	uart_misc_debug("store uart_tx_enable_number 0x%02x \n", uart_tx_enable_number);
	spin_lock(&uart_misc_lock);
...
#define SW_SZ 4
	...
	unsigned int current_val = UART_REG(PM_SLP_RIU, PM_SLP_UART);
	uart_misc_debug("current_val:%X", current_val);
	uart_misc_debug("show uart_tx_enable_number %X \n", uart_tx_enable_number);
	snprintf(buf, SW_SZ, "%x\n", uart_tx_enable_number);
	ret = strlen(buf) + 1;
	return ret;
}
...
static ssize_t uart_rx_enable_sysfs_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	uart_misc_debug("store uart_rx_enable_number 0x%02x \n", uart_rx_enable_number);
	spin_lock(&uart_misc_lock);
...
#define SW_SZ 4
	...
	unsigned int current_val = UART_REG(PM_SLP_RIU, PM_SLP_UART);
	uart_misc_debug("current_val:%X", current_val);
	uart_misc_debug("show uart_rx_enable_number %X \n", uart_rx_enable_number);
	snprintf(buf, SW_SZ, "%x\n", uart_rx_enable_number);
	ret = strlen(buf) + 1;
	return ret;
}
...
static ssize_t uart_tx_enable_sysfs_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count);
static ssize_t uart_tx_enable_sysfs_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf);
static struct kobj_attribute uart_tx_enable[] = { __ATTR(uart_tx_enable, 0660, uart_tx_enable_sysfs_show, uart_tx_enable_sysfs_store),};

static ssize_t uart_rx_enable_sysfs_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count);
static ssize_t uart_rx_enable_sysfs_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf);
static struct kobj_attribute uart_rx_enable[] = { __ATTR(uart_rx_enable, 0660, uart_rx_enable_sysfs_show, uart_rx_enable_sysfs_store),};
...
static int __init uart_misc_init(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(uart_switch); i++)
		if (sysfs_create_file(kobj_ref_misc, &uart_tx_enable[i].attr)) {
			uart_misc_debug("Cannot create sysfs file......\n");
			goto uart_misc_err;
		}

	for (i = 0; i < ARRAY_SIZE(uart_switch); i++)
		if (sysfs_create_file(kobj_ref_misc, &uart_rx_enable[i].attr)) {
			uart_misc_debug("Cannot create sysfs file......\n");
			goto uart_misc_err;
		}
...
uart_misc_err:
	...
	for (i = 0; i < ARRAY_SIZE(uart_switch); i++) {
		sysfs_remove_file(kernel_kobj, &uart_tx_enable[i].attr);
	}
	for (i = 0; i < ARRAY_SIZE(uart_switch); i++) {
		sysfs_remove_file(kernel_kobj, &uart_rx_enable[i].attr);
	}
...
MODULE_DESCRIPTION("Mtk uart misc driver ver 0.01");
