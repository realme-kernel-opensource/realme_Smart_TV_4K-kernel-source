...
int mstar_cust_pwrkey_handler(unsigned int keycode,bool bType)
{
	int i=0;
    pwrkey_handle.type = 0;
	switch (keycode) {
	/* poweron key */
	case 0x46:
	...
	case 0Xf2:
		...
		pwrkey_handle.keycode = keycode;
		break;
	case 0x18D: //BT_RC Netflix KEY_CALENDAR
	case 0x88: //RM BT RC netflix KEY
        case 0xA5: //Haier BT RC netflix key
	    if(bType == 1)//while BT RC case
        {
            pwrkey_handle.nfx_key_cnt ++;
            pwrkey_handle.keycode = keycode;
            pwrkey_handle.type = 1;
        }
		break;
	default:
	...
	case PM_POST_SUSPEND:
		...
		if (!suspend_abort) {
			...
			if (wakeup_source != PM_WAKEUPSRC_CEC
				...
				mstar_cust_pwrkey_handler(key_code,0);
			}
			pwrkey_handle.count ++;
		}
#endif
